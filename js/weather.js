...
async function dataFun() {
  ...
  try {
    ...
    let filteredToday = today.filter(([t]) => Number(t.substring(0, 2)) >= nowHour);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ul.querySelector("li.week-info b").textContent = i === 0 ? "오늘" : dateArr[i].day;
      ul.querySelector("li.week-info small").textContent = dateArr[i].monthDay;
      ...
      const entries = Object.entries(shortFilter[i + 1]).sort(
      ...
      icons[0].textContent = morning
        ? getWeatherIcon({ sky: morning[1].SKY, pty: morning[1].PTY })
        : "help";

      icons[1].textContent = afternoon
        ? getWeatherIcon({ sky: afternoon[1].SKY, pty: afternoon[1].PTY })
        : "help";
    }
    ...
    for (let i = 4; i <= 6; i++) {
      ...
      ul.querySelector("li.week-info b").textContent = dateArr[i].day;
      ul.querySelector("li.week-info small").textContent = dateArr[i].monthDay;

      ul.querySelector(".min").textContent = taItem["taMin" + (i + 1)] + "°";
      ul.querySelector(".max").textContent = taItem["taMax" + (i + 1)] + "°";
